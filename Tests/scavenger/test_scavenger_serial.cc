#include "test_scavenger_serial.h"
...
//
 ...
 IsRegionEq(const Region& lhs, const Region& rhs) {
   if(lhs != rhs)
     return ::testing::AssertionFailure() << lhs << " does not equal " << rhs;
 ...
 TEST_F(SerialScavengerTest, TestSerialScavenge_WillPass_DoesNothing) {
   ...
   auto old_fromspace = zone().GetFromspace();
   auto old_tospace = zone().GetTospace();
   auto a_ptr = malloc(Int32::GetClassAllocationSize());

   ON_CALL(zone_, TryAllocateClassBytes(_)) //TODO: match on this value
     .WillByDefault([&a_ptr](Class* cls) {
       DLOG(INFO) << "returning a_ptr";
       return (uword)a_ptr;
     });

   static constexpr const RawInt32 kAValue = 33;
   auto a = Int32::TryAllocateIn(&zone_, kAValue);
   ...
   ASSERT_FALSE(IsRegionEq(fromspace, old_fromspace));
   ASSERT_TRUE(IsRegionEq(fromspace, old_tospace));
   ASSERT_FALSE(IsRegionEq(tospace, old_tospace));
   ASSERT_TRUE(IsRegionEq(tospace, old_fromspace));
 }
//
